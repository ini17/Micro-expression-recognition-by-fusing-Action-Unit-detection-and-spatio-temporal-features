...
% Date: June 2012
%
function my_butter(vidFile, outDir ...
...
%     vidName = strsplit(vidFile, '\');
%     vidName = vidName{1, 4};
%     fprintf("%s\n", vidName);
% 
%     outName = fullfile(outDir,[vidName '-butter-from-' num2str(fl) '-to-' ...
%         num2str(fh) '-alpha-' num2str(alpha) '.avi']);
...
%     Read images list
    ...
    [length, ~] = size(imglist);
    len = length;
    img_hw = imread([vidFile, imglist(3).name]);
    [h, w, c] = size(img_hw);
    ...
    % firstFrame
    temp.cdata = imread([vidFile, imglist(startIndex).name]);
...
%             temp.cdata = read(vid, i);
            temp.cdata = imread([vidFile, imglist(i).name]);
            [rgbframe,~] = frame2im(temp);
...
%             filename = append(outDir, 'reg_img', num2str(onset+i-4), '.jpg');
            filename = append(outDir, num2str(onset+i-4), '.jpg');
            imwrite(output, filename)
            if i >= endIndex
%                 filename = append(outDir, 'reg_img', num2str(onset+i-3), '.jpg');
                filename = append(outDir, num2str(onset+i-3), '.jpg');
