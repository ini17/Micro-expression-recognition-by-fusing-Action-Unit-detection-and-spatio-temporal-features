...
if __name__ == '__main__':
    # model = P3D199(pretrained=False, num_classes=4)
    model = P3D63(num_classes=4)
    model = model.cuda()
    data = torch.autograd.Variable(
        torch.rand(10, 3, 20, 160, 160)).cuda()  # if modality=='Flow', please change the 2nd dimension 3==>2
    out = model(data)
    print(out.size(), out)
